...
def criar_chamado() -> None:
    local_chamado = input_texto("Para qual local deseja cadastrar um chamado?", tamanho_min=5)
    for local, autor in chamados:
        if local.lower() == local_chamado.lower():
            print(f"[ERRO] {autor} já cadastrou um chamado para esse local!\n")
            return None
    autor_chamado = input_texto("Digite o nome de quem efetuou o chamado", tamanho_max=30)
...
while True:
    ...
    if menu_acoes[idx_opcao_escolhida] == "Cadastrar chamado":
        ...
        idx_chamado_a_finalizar = coletar_indice_menu("Qual chamado deseja finalizar?", listagem_normalizada)
        chamados.pop(idx_chamado_a_finalizar)

        print(f'[-] O chamado "{listagem_normalizada[idx_chamado_a_finalizar]}" foi finalizado')
